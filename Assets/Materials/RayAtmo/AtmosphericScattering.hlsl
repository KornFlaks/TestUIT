...
float4 AtmosphericScattering(
    ...
    const float3 dir, // the direction of the ray (the camera vector)
    const float max_dist, // the maximum distance the ray can travel (because something is in the way, like an object)
    //const float3 scene_color,			// the color of the scene
    const float3 light_dir, // the direction of the light
    float3 light_intensity, // how bright the light is, affects the brightness of the atmosphere
    const float3 planet_position, // the position of the planet
    ...
    float3 beta_ambient,
    // the amount of scattering that always occurs, cna help make the back side of the atmosphere a bit brighter
    const float g,
    ...
    const float atmosphereIntensity,
    const float scatteringIntensity,
    const float4 atmosphereColor
)
{
    // KornFlaks: Obtained from this youtube vid: https://www.youtube.com/watch?v=bFWM8vJZQ_0.
    // Original shader included inside streaming assets.
    // Increasing these hardcoded values will microscopically improve the atmospheric shader graphic at a massive cost.
    const int steps_i = 8;
    const int steps_l = 1;

    const float totalRadius = planet_radius + (atmo_radius * atmosphereIntensity);
    ...
    float t0, t1;
    const bool intersection = rayIntersectsSphere(start, dir, totalRadius, t0, t1);
    ...
    const float2 scale_height = float2(height_ray, height_mie);
    ...
    float phase_mie = allow_mie
                          ? 3.0 / (8 * PI) * ((1.0 - gg) * (mumu + 1.0)) / (pow(abs(1.0 + gg - 2.0 * mu * g), 1.5) * (
                              2.0 + gg))
                          : 0.0;
    ...
    for (int i = 0; i < steps_i; ++i)
    {
        ...
        const float3 pos_i = start + dir * (ray_pos_i + step_size_i * 0.5);
        ...
        rayIntersectsSphere(pos_i, light_dir, totalRadius, t0, t1);
        const float step_size_l = (t1 - t0) / float(steps_l);
        ...
        for (int l = 0; l < steps_l; ++l)
        {
            ...
            density_l.z = clamp((1.0 / cosh((height_absorption - height_l) / absorption_falloff)) * density_l.x, 0.0,
                                1.0);
        ...
        const float3 attn = exp(
            -(beta_mie * (opt_i.y + opt_l.y) + beta_ray * (opt_i.x + opt_l.x) + beta_absorption * (opt_i.z + opt_l.z)));
    ...
    out float4 result)
{
    ...
    float t0, t1;
    const bool intersection = rayIntersectsSphere(start - planet_position, dir, planet_radius, t0, t1);
    //float4 scene = float4(scene_color, 1e12);
    //float3 col = scene.xyz;
    const float maxLength = intersection ? max(t0, 0) : 1e12;

    //if (intersection) { //(intersection.y >= 0) {
    //maxLength = max(t0, 0);//max(intersection.x, 0);
    ...
    // get whether this point is shadowed, + how much light scatters towards the camera according to the lommel-seelinger law
    ...
    result = AtmosphericScattering(start, dir,
                                   //scene.w, scene.xyz,
                                   maxLength,
                                   ...
                                   //steps_i, steps_l,
                                   atmosphereIntensity, scatteringIntensity, atmosphereColor);
}
